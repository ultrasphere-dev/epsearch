...
@pytest.mark.parametrize(
    "boundary",
    [
        CirclesBoundary(center=0, radius=2, radius_min=0.01, n_points=128),
        RectsBoundary(
            center=0j, half_size=2 + 2j, half_size_min=0.01 + 0.01j, n_points_per_side=32
        ),
    ],
)
def test_msd(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        RectsBoundary(
            center=3 + 0j, half_size=2 + 2j, half_size_min=0.01 + 0.01j, n_points_per_side=32
        ),
    ],
)
def test_random(
